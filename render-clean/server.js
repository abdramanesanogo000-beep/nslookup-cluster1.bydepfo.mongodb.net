...
// ===========================================
// PAIEMENT PAYTECH (COMMENTÉ - INTÉGRATION XPAY EN COURS)
// ===========================================
...
// ===========================================
// PAIEMENT XPAY (Paiement Pro)
// ===========================================

const XPAY_API_URL = process.env.XPAY_ENV === 'production' 
    ? 'https://api.paiementpro.net/webservice/onlinepayment/init/curl-init.php'
...
const XPAY_ENV = process.env.XPAY_ENV || 'sandbox';

// Mapper les méthodes de paiement vers les canaux XPay
function mapMethodeToChannel(methode) {
    const mapping = {
        'orange': 'OMCIV2',    // Orange Money CI
        'wave': 'WAVECI',      // Wave CI
        'carte': 'CARD',      // Carte bancaire
        'mtn': 'MOMOCI',      // MTN Mobile Money CI
        'moov': 'MOOVCI'      // Moov CI
    };
    return mapping[methode.toLowerCase()] || 'CARD'; // Par défaut carte bancaire
}

// Initier un paiement XPay
app.post('/api/paiement/initier', async (req, res) => {
    try {
        const { commande_id, montant, client, methode } = req.body;
        ...
        if (!XPAY_MERCHANT_ID) {
            return res.status(500).json({ succes: false, erreur: 'ID Marchand XPay non configuré.' });
        }

        // Générer une référence unique si non fournie
        const referenceNumber = commande_id || `CMD-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;

        // Séparer le nom en prénom et nom
        const noms = client.nom.split(' ');
        const firstName = noms[0] || 'Client';
        const lastName = noms.slice(1).join(' ') || 'Hygia';

        const payload = {
            merchantId: XPAY_MERCHANT_ID,
            amount: Math.round(montant),
            description: `Commande Hygia ${commande_id} - Matériel médical`,
            channel: mapMethodeToChannel(methode),
            countryCurrencyCode: '952', // Code XOF (FCFA)
            referenceNumber: referenceNumber,
            customerEmail: client.email,
            customerFirstName: firstName,
            customerLastname: lastName,
            customerPhoneNumber: client.telephone,
            notificationURL: `${process.env.BACKEND_URL}/api/paiement/xpay-notification`,
            returnURL: `${process.env.FRONTEND_URL}/commande-confirmee.html?ref=${commande_id}`,
        ...
        console.log('XPay Initialisation Request:', payload);
        ...
        const data = await response.json();
        console.log('XPay Initialisation Response:', data);

        if (data.success && data.url) {
            // Mettre à jour la commande avec les infos XPay
            await Commande.findOneAndUpdate(
                { numero: commande_id },
                {
                    $set: {
                        xpay_reference: referenceNumber,
                        statut: 'En attente paiement',
                        paiement_confirme: false
                    }
                }
        ...
        console.error('Erreur XPay initialisation :', data);
        return res.status(400).json({ 
            succes: false, 
            erreur: data.message || 'Erreur initialisation paiement XPay' 
        });
    } catch (error) {
        console.error('Erreur POST /api/paiement/initier (XPay) :', error);
        return res.status(500).json({ succes: false, erreur: 'Erreur serveur lors de l\'initialisation XPay' });
    }
});

// Webhook XPay — notification automatique après paiement
app.post('/api/paiement/xpay-notification', async (req, res) => {
    try {
        const { merchantId, referenceNumber, amount, responsecode, returnContext } = req.body;

        console.log('XPay Webhook reçu:', req.body);

        if (!referenceNumber) {
            return res.status(200).json({ status: 'ok' });
        }

        // Trouver la commande par référence XPay
        const commande = await Commande.findOne({ xpay_reference: referenceNumber });

        if (!commande) {
            console.log('⚠️ Webhook XPay : commande introuvable pour référence ' + referenceNumber);
            return res.status(200).json({ status: 'ok' });
        }

        // responsecode: 0 = succès, -1 = échec
        if (responsecode === '0') {
            const commandeConfirmee = await Commande.findOneAndUpdate(
                { xpay_reference: referenceNumber },
                { 
            ...
            console.log('✅ Paiement XPay confirmé : ' + commande.numero);
            ...
            console.log('❌ Paiement XPay échoué : ' + commande.numero + ' (code: ' + responsecode + ')');
        }

        return res.status(200).json({ status: 'ok' });
    } catch (error) {
        console.error('Erreur POST /api/paiement/xpay-notification :', error);
        return res.status(200).json({ status: 'ok' });
    }
});
